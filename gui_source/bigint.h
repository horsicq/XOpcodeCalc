...
BigInt big_random(size_t num_digits = 0)
{
    ...
    while (big_rand.value.size() < num_digits) big_rand.value += std::to_string(rand_generator());
    if (big_rand.value.size() != num_digits) big_rand.value.erase(num_digits);  // erase extra digits
...
{
    if (num < 0) throw std::invalid_argument("Cannot compute square root of a negative integer");
...
BigInt BigInt::operator*(const BigInt& num) const
{
    ...
    BigInt product;
    if (abs(*this) <= FLOOR_SQRT_LLONG_MAX and abs(num) <= FLOOR_SQRT_LLONG_MAX) product = std::stoll(this->value) * std::stoll(num.value);
    ...
    if (this->sign == num.sign) product.sign = '+';
    else product.sign = '-';
...
BigInt BigInt::operator/(const BigInt& num) const
{
    ...
    BigInt quotient;
    if (abs_dividend <= LLONG_MAX and abs_divisor <= LLONG_MAX) quotient = std::stoll(abs_dividend.value) / std::stoll(abs_divisor.value);
    else if (abs_dividend == abs_divisor) quotient = 1;
...
BigInt BigInt::operator%(const BigInt& num) const
{
    ...
    BigInt remainder;
    if (abs_dividend <= LLONG_MAX and abs_divisor <= LLONG_MAX) remainder = std::stoll(abs_dividend.value) % std::stoll(abs_divisor.value);
    else if (abs_dividend < abs_divisor) remainder = abs_dividend;
