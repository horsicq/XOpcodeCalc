...
BigInt BigInt::operator*(const BigInt& num) const
{
    ...
    BigInt product;
    if (abs(*this) <= FLOOR_SQRT_LLONG_MAX && abs(num) <= FLOOR_SQRT_LLONG_MAX) product = std::stoll(this->value) * std::stoll(num.value);
...
BigInt BigInt::operator/(const BigInt& num) const
{
    ...
    BigInt quotient;
    if (abs_dividend <= LLONG_MAX && abs_divisor <= LLONG_MAX) quotient = std::stoll(abs_dividend.value) / std::stoll(abs_divisor.value);
...
BigInt BigInt::operator%(const BigInt& num) const
{
    ...
    BigInt remainder;
    if (abs_dividend <= LLONG_MAX && abs_divisor <= LLONG_MAX) remainder = std::stoll(abs_dividend.value) % std::stoll(abs_divisor.value);
